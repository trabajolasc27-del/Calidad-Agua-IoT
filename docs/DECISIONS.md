...
**Cómo se corrigió:** se agregó el embed `measurements!first_measurement_id(value)` a las consultas de `alerts.service.ts` (listado) y `dashboard.service.ts` (`getActiveAlerts`), se extendieron los modelos `Alert`/`DashboardAlert` con el valor, y se muestra en `alerts.html`, `alert-detail-dialog.html` y `dashboard.html`. Verificado que la sintaxis del embed es válida contra el esquema real (petición REST anónima devolvió `200 []` en vez de un error de relación) y que `ng build`/`tsc` quedan limpios; no se pudo re-verificar visualmente con una sesión autenticada porque la sesión de navegador de la sesión anterior había expirado y no corresponde volver a iniciar sesión escribiendo la contraseña por el usuario.
