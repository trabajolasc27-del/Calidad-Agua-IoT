...
- Edge Functions `ingest-measurement` y `admin-create-user` reescritas con los nuevos nombres de tabla/columna/función y el nuevo contrato JSON.
...
**Qué falta (no bloqueante):** no se pudo hacer un recorrido en vivo completo por el navegador dentro de esta misma tanda de cambios porque el resto de la migración (Edge Functions, firmware, frontend) se completó primero; queda pendiente la verificación funcional de punta a punta antes de dar esto por cerrado del todo — ver [PROGRESS.md](PROGRESS.md).
